...
| `title` | 実績名 |
| `description` | 達成条件の説明 |
| `category` | `distance`, `prefecture`, `municipality` など |
| `condition` | 達成条件 |
...
| カテゴリ | 例 |
| --- | --- |
| 総移動距離 | 1km、10km、100km、500km、1000km |
...
`odo-earth-*` は地球一周相当など、特別な距離トロフィーとして通常距離実績とは別の表示名を付けられるようにする。
...
ユーザーが通知を許可している場合、実績解除時はフォアグラウンド・バックグラウンドを問わず必ずローカル通知を送る。通知権限がない場合は通知をスキップし、フォアグラウンド中であればアプリ内演出のみ表示する。
...
`Xで自慢する` は、共有文言を作ってOS標準共有UIまたはX投稿URLを開く。

初期実装ではOS標準共有UIを優先する。Xアプリがない環境でもユーザーが共有先を選べるためである。
