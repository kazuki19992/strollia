...
import { createAchievementStyles } from '@/app/styles/achievementStyles';
import { createCommonStyles } from '@/app/styles/commonStyles';
import { createDailyLogStyles } from '@/app/styles/dailyLogStyles';
import { createMapStyles } from '@/app/styles/mapStyles';
import { createSettingsStyles } from '@/app/styles/settingsStyles';
...
describe('appStyles 分割後のキー網羅検証', () => {
  ...
  describe('createDailyLogStyles — 日別ログ・スライダー・GIF', () => {
    test('日別ログ・ルートマップ・スライダー・GIF・共有の代表キーを持つ', () => {
      ...
      expect(styles.shareButtonWideBase).toBeDefined();
  ...
  describe('createStyles — 全体結合', () => {
    ...
    test('分割前の全358キーが存在する（過不足なし）', () => {
      const styles = createStyles(theme);
      const keys = Object.keys(styles);

      // 分割前の総キー数と一致することを確認する（将来のキー追加で差分が出た場合に検知できる）
      expect(keys.length).toBe(358);
    });
  });
});
