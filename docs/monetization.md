...
滞在場所は無料版で1件、Plus有効時は複数件を利用できる。登録済みの設定は契約状態にかかわらずSQLiteに保持し、無料版または解約中は作成順で最初の1件だけを有効にする。再契約時は保存済み全件を自動的に有効へ戻す。

有効な滞在場所は、記録中のGPS揺れを中心座標へ吸着する対象と、共有用の地図で非表示範囲を適用する対象に共通で使う。アプリ内の通常地図、端末内のGPSログ、GPXエクスポートは非表示範囲の対象外とする。
