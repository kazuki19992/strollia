...
- **キャッシュ保存に失敗した場合は、その回の走査結果をそのまま表示する**（レビュー指摘を受けて 2026-08-23 に追加）。
  `photo_assets` が空のままビューポート検索へ進むと、走査できているのに1枚も表示されない回帰になるため。
  フォールバック時もメモリ上で同じ緯度経度境界へ絞り込み、表示件数を増やさない
- シーケンス番号（読み込みの取り消し判定）は**実際に読み込みを開始する時点でだけ**進める。
  余白内へ戻る早期 return で進めると、実行中の読み込みが結果を捨てて `fetchedBoundsRef` が古いまま残る

`usePhotoMapCrashBreaker` のクラッシュブレーカー動作は**変更しないこと**。ただし保存済み設定の復元時には
`getPermissionsAsync()`（参照のみ・ダイアログを出さない）でフルアクセスを再確認し、満たさない場合は
写真表示を OFF へ戻す（`resetPhotoMapPersistedState` を再利用。復元経路では Alert を出さない）。
フルアクセスで作ったキャッシュが、限定アクセスへ変更後も地図へ再表示されるのを防ぐため
（レビュー指摘を受けて 2026-08-23 に追加）。
...
- `hasNextPage` が true: `taken_at > 下限` の行のみ対象（**下限は排他**）。`taken_at` が NULL の行は対象外（安全側）
  - 撮影日時は一意なカーソルではないため、同一時刻の写真がページ境界をまたぐと、両端閉区間（`>=`）では
    次ページ側の**実在するが未走査の写真**まで削除してしまう。境界時刻の行を残す代わりに実在する写真を
    消さない、という意図的な安全側の判断（レビュー指摘を受けて 2026-08-23 に修正）
