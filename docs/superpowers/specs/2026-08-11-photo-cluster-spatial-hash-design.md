...
**既存の「上限なし」性質の扱い**: `getPhotoClusterRadiusMeters` は現在、広域ズームで際限なく半径が伸びる(既存テストで明示的に検証済み)。この設計では段階制に変えるため厳密には有限になるが、`latitudeDelta` の現実的な上限(地図が表現できる最大範囲、~180〜360度)を十分にカバーする巨大な最終段階を用意することで、UI 上は「実質無制限」と区別がつかない挙動にする。既存の「上限なし」テストは、新しい段階値に合わせて書き換える(意図した変更であり回帰ではない)。
...
**この設計により、現行 O(N²) 実装とビット単位で同一の出力になる。** 索引は候補を絞るためだけに使い、距離判定・採用基準は一切変更しないため。
