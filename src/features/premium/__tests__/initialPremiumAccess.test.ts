...
describe('起動時のPlus状態取得', () => {
  ...
  test('取得が完了した場合はタイマーを残さず結果を返す', async () => {
    ...
    await expect(resolveInitialPremiumAccess(Promise.resolve(active), fallback, 3000)).resolves.toBe(active);
    expect(jest.getTimerCount()).toBe(0);
  });
  ...
  test('取得が失敗した場合はタイマーを残さずフォールバックを返す', async () => {
    ...
    await expect(resolveInitialPremiumAccess(Promise.reject(new Error('取得失敗')), fallback, 3000)).resolves.toBe(fallback);
    expect(jest.getTimerCount()).toBe(0);
  });
});
