...
地図種別、写真表示、滞在場所表示は、メインマップの補助操作として扱う。

- 標準マップ / 航空写真を切り替える
- 写真表示は初期OFF
- 滞在場所を1件以上登録している場合だけ「マップ上に滞在場所を表示」を表示する。メインマップの「マップの表示」では写真表示の直下、設定画面では写真関連項目の後かつ「マップのテーマ」の直前に置く
- 滞在場所表示は初期ONとし、OFFではマーカーだけを隠す。GPS吸着や共有時の非表示処理には影響させない
...
- GPXインポート
- マップ上の写真表示設定
- マップ上の滞在場所表示設定（滞在場所を1件以上登録している場合のみ）
- 画面ON維持設定
- Strollia Plus
- OSSライセンス
- 全データ削除
