...
`;
    ...
    expect(patchFmtPodspecContents(podspec)).toContain('"CLANG_CXX_LANGUAGE_STANDARD" => "c++17"');
  });
...
`;
    ...
    expect(() => patchFmtPodspecContents(podspec)).toThrow('with-fmt-cxx17: expected fmt C++ standard setting was not found');
  });
});
