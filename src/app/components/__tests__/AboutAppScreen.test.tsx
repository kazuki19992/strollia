...
describe('このアプリについて画面 AboutAppScreen', () => {
  ...
  test('ローカルファーストとプライバシーを含む本文初稿を表示する', () => {
    ...
    expect(texts).toContain('すとろりあは、毎日の移動や散歩の足あとを端末に残していくGPSロガーです。地図を埋めたり、日々の距離を振り返ったりしながら、自分だけの移動記録を育てていけます。');
    expect(texts).toContain('GPSログや移動履歴は端末内に保存します。ユーザーの明示操作なしに、移動履歴や写真メタデータを外部へ送信しません。');
    expect(texts).toContain('現在地を利用する機能を追加する場合があります。その場合も、機能を明示的に有効にしたときだけ、必要な現在地情報を外部サービスへ送信する設計にします。移動履歴をサーバーに保存することはありません。');
    expect(texts).toContain('Strollia Plusは、記録をもっと楽しく便利にするための追加機能です。基本の記録体験と、ユーザー自身がデータを扱えることを大切にします。');
  });
});
