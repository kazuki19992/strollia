...
- 更新項目は重要順に1件以上を定義し、1件につき10文字以内で、必ず1項目を1行に収める。看板は先頭2件だけを表示する
...
- 定義が `null`、過去版／将来版との版不一致、項目0件、空文字または11文字以上の項目を安全に非表示へ落とす
...
2. アプリ内候補は各10文字以内とし、重要順にすべて `items` へ指定する。UIは先頭2件と、3件目以降がある場合の「など……」を自動表示する
