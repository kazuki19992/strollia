...
describe('設定リポジトリ settingsRepository', () => {
  ...
  it('文字列設定が保存されている場合はその値を返す', async () => {
    (db.getFirstAsync as jest.Mock).mockResolvedValue({ value: JSON.stringify('glow') });

    await expect(getStringSetting('routeLineStyle', 'classic')).resolves.toBe('glow');
  });

  it('文字列設定が壊れている場合はfallbackを返す', async () => {
    (db.getFirstAsync as jest.Mock).mockResolvedValue({ value: '{broken' });

    await expect(getStringSetting('routeLineStyle', 'classic')).resolves.toBe('classic');
  });
