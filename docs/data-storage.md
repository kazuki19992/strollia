...
旧バージョンが保存した読み込み可能な絶対URIは起動時に管理領域へ移行する。移行後の設定保存に失敗した場合は移行ファイルを削除し、そのセッションでは有効な旧URIを引き続き表示する。画像の読込エラーや参照先消失時は、SQLiteの選択値や参照を削除せず、そのセッションの表示だけOS標準へフォールバックする。
