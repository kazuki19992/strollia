...
- 既存GPSポイントと競合する場合は、既存データを優先する
- 取り込み完了時に、取り込み件数とスキップ件数を表示する
- 設定画面に、GPXのみ対応することと既存データ優先で取り込むことを記載する
...
重複や競合が見つかった場合は、インポート側で既存データを変更しない。Strolliaがすでに持っているGPSログを正とし、GPX側の点はスキップ件数へ含める。
...
XMLパースには `fast-xml-parser` を使う。GPX専用ライブラリも候補にできるが、今回必要なのは `trkpt`、`time`、`ele` の抽出に限定される。GPXライブラリの独自データ構造や距離計算へ依存せず、XMLから必要な値だけを型付きで取り出す方針にする。
...
設定画面では、同じ時刻・座標のデータがすでにある場合は既存データを優先し、GPX側の重複点は追加しないことも明記する。
