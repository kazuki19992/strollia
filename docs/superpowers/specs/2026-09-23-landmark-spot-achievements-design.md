...
| メンバー     | スポット1地点              | 路線を分割した1区間   |
| 達成判定     | 半径内に滞在時間だけ留まる | 区間を通過する        |
| 進捗         | 到達数 / 有効スポット数    | 走破区間数 / 総区間数 |

**メンバーを「区間」として数えれば、走破率も分数で表せる**（120/240区間）。
そのため進捗の表現、プログレスバー、完走実績、パック一覧・詳細のUIは**そのまま流用できる**。

差し替えが必要なのは**達成判定の層だけ**である。

### 12.2 共通化できるもの

以下は高速道路実績でも同じ形で使えるため、スポット固有の前提を埋め込まない。

| 対象                                | 注意点                                                                             |
| ----------------------------------- | ---------------------------------------------------------------------------------- |
| `AppProgressBar`                    | 割合と色だけを受け取る汎用コンポーネントとして作る。スポット固有の型を引数にしない |
| `AppListItem` の `footer` スロット  | 汎用スロットとして追加する。「パック行専用」にしない                               |
| コレクション一覧・詳細のUI構造      | 一覧行（トロフィー＋名前＋説明＋進捗バー）→ 詳細（メンバー一覧）という構造は同一   |
| JSON + JSON Schema + 生成スクリプト | 同じパイプラインを路線データにも使う                                               |
...
以下はスポット実績と根本的に異なるため、**流用を前提にしない**。

| 対象             | 違い                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------ |
| マスタデータの形 | 点（中心座標＋半径）ではなく線（路線のポリライン）                                                           |
| 達成判定         | `resolveLandmarkArrival`（距離＋滞在時間）は使えない。線の被覆判定が必要                                     |
| 訪問記録テーブル | `landmark_spot_visits` は `spot_id` を主キーに1到達1行。路線は区間ごとに数千行規模になるため別テーブルにする |
| データ作成方法   | 61件の手入力とは規模が違う。路線形状データの出所とライセンスを別途検討する必要がある                         |

### 12.4 高速道路実績の設計時に検討すること

- 区間の分割方法。既存の Visited Grid（`src/features/location/grid/`）と重ね合わせる方式が使えるか
- 並走する一般道を走っただけで誤判定しないための条件（速度・連続性・高度）
