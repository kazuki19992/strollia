...
- 重大な例外やクラッシュの解析のため、Sentryへスタックトレース、アプリ/ビルド情報、OS/端末情報、画面名、RevenueCatのSupport ID、サブスク加入状況などの診断情報を送信する場合。送信項目の詳細は `docs/architecture.md` のSentry送信項目に従い、GPSログ本体、写真ジオタグ、座標値は送信しない
