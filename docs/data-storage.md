...
保存時はGPSの生データをそのまま入れるのではなく、明らかに精度が低い点と、前回保存点からの移動が小さい点を破棄する。

保存判定ではGPS速度または前回保存点からの経過時間で推定した速度をもとに、停止中・徒歩・車両移動・高速移動に分類する。停止中や徒歩では精度を重視してGPS揺れを破棄し、車両移動や高速移動では移動距離が十分に大きい場合のみ多少精度が悪くても保存する。

初期閾値は以下とする。

- 初回ポイントは水平方向の位置精度が50m以内の場合のみ保存する
- どの移動モードでも水平方向の位置精度が80mを超える点は保存しない
- 停止中は20m未満の移動、または精度が30mを超える点を保存しない
- 徒歩中は5m未満の移動、精度が35mを超える点、または精度円に対して小さすぎる移動を保存しない
- 車両移動中は15m未満の移動、精度が50mを超える点、または精度円に対して小さすぎる移動を保存しない
- 高速移動中は30m未満の移動、または精度円に対して小さすぎる移動を保存しない
