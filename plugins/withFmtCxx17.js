...
const PODSPEC_RELATIVE_PATH = path.join('node_modules', 'react-native', 'third-party-podspecs', 'fmt.podspec');
const FMT_CXX_STANDARD_PATTERN = '"CLANG_CXX_LANGUAGE_STANDARD" => rct_cxx_language_standard()';
...
function patchFmtPodspecContents(contents) {
  ...
  if (!contents.includes(FMT_CXX_STANDARD_PATTERN)) {
    throw new Error(`${PLUGIN_NAME}: expected fmt C++ standard setting was not found`);
  }

  return contents.replace(FMT_CXX_STANDARD_PATTERN, FMT_CXX_STANDARD_REPLACEMENT);
}
...
function withFmtCxx17(config) {
  return withDangerousMod(config, [
    'ios',
    (modConfig) => {
      const podspecPath = path.join(modConfig.modRequest.projectRoot, PODSPEC_RELATIVE_PATH);
