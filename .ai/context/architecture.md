...
expo-router ルート (src/app/)
  ...
  index.tsx / monthly-report.tsx — トップレベル画面
  achievements/_layout.tsx + index.tsx + [packId].tsx — 実績スタック
...
| `src/app/achievements/`             | 実績スタック。`_layout.tsx` + `index.tsx`(一覧 `/achievements`) + `[packId].tsx`(スポットパック詳細)                                                                                                                                                                                                                                                            |
...
| `src/ui/hooks/`                     | カスタムフック群。主要フック: `useAppInitialization`(起動初期化), `useLocationRecordingSync`(GPS記録同期), `useAchievementState`(実績), `usePremiumAccess`(課金), `useMapFollowState`(地図追従), `useVisitedGridOverlay`(訪問グリッド), `useUserLocationIconSetting`(アイコン), `usePhotoMapCrashBreaker`(写真表示), `useLandmarkPackState`(スポットパック進捗) |
...
| `src/features/landmarks/`           | スポット訪問実績(Plus限定)。マスタは `data/landmarks/` のJSONから `npm run generate:landmarks` で `landmarkCatalog.generated.ts` を生成する。到達判定(`landmarkArrivalResolver.ts`)・到達記録(`landmarkVisitRepository.ts`)・記録時の境界(`landmarkRecordingService.ts`)・到達通知(`landmarkNotificationService.ts`)                                            |
